...
exports.run = async (FM, args, reply, lng, FN) => {
  ...
  const Request = await Server.searchCosmetic(query, 'pickaxe')
  if (Request.data) {
  ...
  await FN.party.me.setPickaxe(query)
  FN.currentLoadout.pickaxe = query
  if (FN.currentLoadout.emote) {
    await FN.party.me.clearEmote()
    await FN.party.me.setEmote(FN.currentLoadout.emote)
  };
  return await reply(i18next.t('msg_pxchanged', { ns: 'bot', lng, pickaxe: displayName }))
}
